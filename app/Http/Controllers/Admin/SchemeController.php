...
class SchemeController extends Controller
{
    ...
    public function updateScheme(Request $request){
        ...
        if($filename){
            $scheme = Scheme::where('id',$id)->update(['scheme_image' => $filename,]);
        }
    ...
    public function manageAppliedScheme(Request $request){
        if(auth()->user()->role_id == 4){
            ...
            ->orderBy('applied_schemes.updated_at','DESC')
            ->get();
    ...
    public function appliedScheme(Request $request){
        ...
        if(!empty($farmer) && $request->accept == 'accept'){
            ...
            if($farmer->stage == 'Tehsil'){
                if($status == 'Resubmit'){
                    $updateFarmer = AppliedScheme::where('id', $request->id)->update(['tehsil_updated'=>date('Y-m-d H:i:s'), 'status'=>'Resubmit', 'stage' => 'Tehsil','reason' => $reasons,'approved_tehsil' => auth()->user()->id,'attempts'=>($farmer->attempts +1)]);
                    ...
                    $updateFarmer = AppliedScheme::where('id', $request->id)->update(['tehsil_updated'=>date('Y-m-d H:i:s'), 'status'=>'Rejected', 'stage' => 'State','approved_tehsil' => auth()->user()->id,'reason' => $reasons]);
                    if($updateFarmer){
                        $stateInfo = $user->officerstate();
                        if(!empty($stateInfo)){
                            foreach($stateInfo as $state){
            ...
            elseif($farmer->stage == 'District'){
                if($status == 'Resubmit'){
                    $updateFarmer = AppliedScheme::where('id', $request->id)->update(['district_updated'=>date('Y-m-d H:i:s'), 'district_status'=>'Resubmit', 'stage' => 'District','district_reason' => $reasons,'approved_district' => auth()->user()->id,'attempts'=>($farmer->attempts +1)]);
                    if($updateFarmer){
                        Notification::create([
                            'user_id' => 1,
                    ...
                    $updateFarmer = AppliedScheme::where('id', $request->id)->update(['district_updated'=>date('Y-m-d H:i:s'), 'district_status'=>'State', 'stage' => 'District','district_reason' => $reasons,'approved_district' => auth()->user()->id]);
                    if($updateFarmer){
                        Notification::create([
                            'user_id' => 1,
