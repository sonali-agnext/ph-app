             ...
             <div class="card">
                <div class="card-body">
                        ...
                        @endif
                            ...
                            <form class="row g-3" method="POST" action="{{ route('update-scheme-subcategory',['id' => $scheme_sub_category->id]) }}">
                                @csrf
                                <div class="col-md-12">
                                    <div class="form-floating mb-3">
                                        <select class="form-select" name="scheme_category_id" id="scheme_category_id" aria-label="Scheme Category">
                                            ...
                                            <option @if($dst->id == $scheme_sub_category->scheme_category_id) {{ __('selected') }}@endif value="{{ $dst->id }}">{{$dst->category_name}}</option>                                            
                                            @empty
                                            @endforelse
                                        </select>
